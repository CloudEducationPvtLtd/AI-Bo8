...
const Header = () => {
  ...
  return (
    <div className={`fixed top-0 left-0 w-full z-50 border-b border-n-6 lg:bg-n-8/90 lg:backdrop-blur-sm ${openNavigation ? 'bg-n-8' : 'bg-n-8/90 backdrop-blur-sm'}`}>
        ...
        <a href="#hero" className="block w-[12rem] xl:mr-8"><img src={aibo8} alt="AiBo8" width={190} height={40} /></a>
            ...
            <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
                {navigation.map((item) => (
                   item.url === "/#shop" ? (
                    <a
                      key={item.id}
                      href={item.url}  // Use href for <a> tag instead of to
                      onClick={handleClick}
                    ...
                    <Link
                      key={item.id}
                      to={item.url}  // Use to for <Link> component
                      onClick={handleClick}
                      className={`block relative font-code text-2xl uppercase text-n-1 transition-colors hover:text-color-1 ${item.onlyMobile ? "lg:hidden" : ""} px-6 py-6 md:py-8 lg:-mr-0.25 lg:text-xs lg:font-semibold ${item.url === pathname.hash ? 'z-2 lg:text-n-1' : 'lg:text-n-1/50'} lg:leading-5 lg:hover:text-n-1 xl:px-12`}
                    >
                      {item.title}
        ...
        <Link to='/about' className="hidden button mr-8 text-n-1/50 transition-colors hover:text-n-1 lg:block">
        About
        </Link>
